...
- For an existing schema, bind to the reviewed datasource JNDI name (for example, `@Repository(dataStore = "jdbc/<name>")`, replacing the `<name>` placeholder with the configured datasource name) or the explicit persistence-unit reference selected by the contract. Capture SQL and compare catalog/schema/table names before editing the database.
