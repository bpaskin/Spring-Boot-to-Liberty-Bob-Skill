...
```bash
grep -rn --include='*.java' --include='*.properties' --include='*.yml' --include='*.xml' --include='*.md' --include='*.json' \
...
>
> ```
> Migrate Spring Boot to Jakarta EE 11 + Open Liberty
>
> Migrated by IBM Bob using skill migrate-spring-to-liberty
> ```
>
> Proceed?
...
gh pr create --draft \
  --title "<branch-name>: Spring Boot → Jakarta EE 11 + Open Liberty migration" \
